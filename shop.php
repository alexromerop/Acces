...
EOD;
		...
		$query = <<<EOD
SELECET * FROM user_products
WHERE id_user={$_SESSION["id_usser"]} AND id_product ={$prod["id_product"]};
EOD;
		$res = $conn->query($query);
		if($res){
			if ($res->num_rows == 0){
...
<input type="hiden" name="id_product" value="{$prod["id_product"]}" />
...
EOD;
  ...
  <p><strong>Reference</strong>: {$prod["reference"]}</p>
	<p><strong>Author</strong>: {$prod["id_group"]}</p>
	<p><strong>Engine</strong>: {$prod["id_engine_version"]}</p>
EOD;

}
else{
	$content = "No hay producto con esa refenrecia";
	}
